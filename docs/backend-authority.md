...
- A successful run command response must include both a valid authoritative run and a non-array `result` object before its browser receipt is removed. A missing/malformed result remains unconfirmed even when the returned run looks valid; retry replays the same operation instead of losing its recorded answer or sending a new action. Status-only responses need no `result`.
- Terminal recovery is single-flight per live view through both transport and answer presentation. Overlapping retries cannot display the same recovered answer twice; an inactive view cannot change loading/readiness or release a replacement view's recovery gate. Failed recovery releases only its own gate so the player can retry.
