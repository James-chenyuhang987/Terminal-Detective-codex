const MODULE_META = {
  zh: {
    ...
    tech: ['科技研发', '永久能力将叠加到探员技能'],
    comms: ['探员通讯', '系统通知与单机剧情联络'],
    ...
    agent_market: ['探员市场', '使用游戏内钻石签约高阶支援探员'],
    agents: ['探员名册', '三名核心探员与已签约的支援成员'],
    settings: ['设置中心', '游戏、画面、语言与账户设置'],
  },
  en: {
    ...
    tech: ['RESEARCH', 'Permanent upgrades stack with agent skills'],
    comms: ['COMMS', 'System notices and single-player story contacts'],
    ...
    agent_market: ['AGENT MARKET', 'Recruit advanced support agents with earned diamonds'],
    agents: ['AGENT ROSTER', 'Your three core agents and recruited support members'],
    settings: ['SETTINGS', 'Game, visuals, language and account'],
  },
};

const MODULE_ICONS = {
  profile: 'id', level_road: 'route', supply: 'bolt', diamonds: 'gem', warehouse: 'briefcase',
  graph: 'network', tech: 'settings', comms: 'mail', intel: 'book', cases: 'archive',
  achievements: 'trophy', checkin: 'calendar', events: 'gift', tutorial: 'book', goals: 'target',
  agent_market: 'users', agents: 'detective', settings: 'sliders',
};
...
export function getHomeModuleMeta(moduleKey, lang = 'zh') {
  ...
  return {
    title,
    subtitle,
    icon: MODULE_ICONS[moduleKey] || MODULE_ICONS.warehouse,
