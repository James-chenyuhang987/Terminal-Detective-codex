  ...
  return <>
    ...
    <Panel>
      ...
      <div style={{ display: 'flex', gap: 9, marginTop: 12, flexWrap: 'wrap' }}>
        <ActionButton disabled={busy || profile.energy >= ENERGY_OVERFLOW_MAX} style={!canBuyDirect || profile.energy >= ENERGY_OVERFLOW_MAX ? { opacity: .52 } : {}} onClick={() => onApply({ type: 'buy_use_energy_cell' }, purchaseSuccessMessage(`${lang === 'zh' ? '体力' : 'Energy'} +${directRestore}`, lang))}>🪙 400 · {lang === 'zh' ? '购买并使用' : 'BUY & USE'}</ActionButton>
...
function WarehouseModule({ profile, onApply, lang, tx, busy }) {
  ...
  return <>
    ...
    <div style={{ display: 'grid', gap: 10 }}>{ITEM_CATALOG.filter(item => tab === 'shop' || (tab === 'equipped' ? profile.equipped_items.includes(item.id) : profile.inventory[item.id] > 0)).map(item => {
      ...
      return <Panel key={item.id} accent={item.currency === 'gold' ? '#e8c98a' : '#5fd8ff'}><div className="td-store-item"><span className="td-store-item-icon">{item.icon}</span><div className="td-store-item-copy"><div style={{ fontWeight: 900 }}>{text.name}</div><div style={{ color: 'rgba(255,255,255,.42)', fontSize: '.56rem', marginTop: 4 }}>{text.desc}</div><div style={{ color: '#7df1ff', fontSize: '.56rem', marginTop: 5 }}>{lang === 'zh' ? '持有' : 'OWNED'} × {profile.inventory[item.id]} / {item.stackLimit}</div></div>{tab === 'shop' ? <div className="td-store-buy"><QuantityPicker value={quantity} max={room} onChange={value => setQuantities(current => ({ ...current, [item.id]: value }))} /><ActionButton style={!quote.canPurchase ? { opacity: .52 } : {}} accent={item.currency === 'gold' ? '#e8c98a' : '#5fd8ff'} onClick={() => onApply({ type: 'purchase_item', item_id: item.id, quantity: quantity }, purchaseSuccessMessage(`${text.name} ×${quantity}`, lang))}>{item.currency === 'gold' ? '🪙' : '💎'} {quote.totalCost || item.cost}</ActionButton>{!quote.canPurchase && <small>{quote.error === 'inventory_full' ? (lang === 'zh' ? '库存已满' : 'FULL') : (lang === 'zh' ? '资源不足' : 'INSUFFICIENT FUNDS')}</small>}</div> : item.id === 'energy_cell' ? <ActionButton disabled={busy || Number(profile.inventory.energy_cell) < 1 || profile.energy >= ENERGY_OVERFLOW_MAX} onClick={() => onApply({ type: 'consume_energy_cell' }, lang === 'zh' ? '体力 +30' : 'Energy +30')}>{tx.use}</ActionButton> : <ActionButton onClick={() => onApply({ type: 'equip_item', item_id: item.id }, equipped ? (lang === 'zh' ? '已卸下' : 'Removed') : (lang === 'zh' ? '已装备' : 'Equipped'))}>{equipped ? tx.unequip : tx.equip}</ActionButton>}</div></Panel>;
    })}</div>
