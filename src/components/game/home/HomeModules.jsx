...
function ProfileModule({ profile, onApply, tx, lang }) {
  ...
  return <>
    ...
    <Panel>
      ...
      <ActionButton onClick={() => onApply(current => editIdentity(current, { detective_name: name, avatar, signature, identity_badge: badge, detective_tags: tags }), lang === 'zh' ? '档案已同步' : 'Profile synced')}>{tx.save}</ActionButton>
    </Panel>
...
function AgentMarketModule({ profile, onApply, onOpenModule, onEnterLobby, lang }) {
  ...
  return <>
    ...
    <div className="td-agent-market-grid">{coreAgents.map(agent => <AgentCard key={agent.id} agent={agent} lang={lang} market profile={profile} owned={ownedIds.has(agent.id)} onBuy={selected => onApply(current => purchaseAgent(current, selected.id), purchaseSuccessMessage(lang === 'zh' ? `${selected.zh.name} · 已加入核心候选` : `${selected.en.name} · Added to core reserves`, lang))} />)}</div>
    ...
    <div className="td-agent-market-grid">{supportAgents.map(agent => <AgentCard key={agent.id} agent={agent} lang={lang} market profile={profile} owned={ownedIds.has(agent.id)} active={getActiveSupportAgentId(profile) === agent.id} onBuy={selected => onApply(current => purchaseAgent(current, selected.id), purchaseSuccessMessage(lang === 'zh' ? `${selected.zh.name}${getActiveSupportAgentId(profile) ? '' : ' · 已设为支援'}` : `${selected.en.name}${getActiveSupportAgentId(profile) ? '' : ' · Active support'}`, lang))} />)}</div>
...
function OwnedAgentsModule({ profile, onApply, onOpenModule, onEnterLobby, lang }) {
  ...
  return <>
    ...
    <div className="td-agent-market-grid">{agents.map(agent => <AgentCard key={agent.id} agent={agent} lang={lang} profile={profile} owned active={activeId === agent.id} onActivate={selected => onApply(current => activateSupportAgent(current, selected.id), lang === 'zh' ? `${selected.zh.name} 已设为当前支援` : `${selected.en.name} is now active support`)} />)}</div>
...
function WarehouseModule({ profile, onApply, lang, tx, busy }) {
  ...
  return <>
    ...
    <div style={{ display: 'grid', gap: 10 }}>{ITEM_CATALOG.filter(item => tab === 'shop' || (tab === 'equipped' ? profile.equipped_items.includes(item.id) : profile.inventory[item.id] > 0)).map(item => {
      ...
      return <Panel key={item.id} accent={item.currency === 'gold' ? '#e8c98a' : '#5fd8ff'}><div className="td-store-item"><span className="td-store-item-icon">{item.icon}</span><div className="td-store-item-copy"><div style={{ fontWeight: 900 }}>{text.name}</div><div style={{ color: 'rgba(255,255,255,.42)', fontSize: '.56rem', marginTop: 4 }}>{text.desc}</div><div style={{ color: '#7df1ff', fontSize: '.56rem', marginTop: 5 }}>{lang === 'zh' ? '持有' : 'OWNED'} × {profile.inventory[item.id]} / {item.stackLimit}</div></div>{tab === 'shop' ? <div className="td-store-buy"><QuantityPicker value={quantity} max={room} onChange={value => setQuantities(current => ({ ...current, [item.id]: value }))} /><ActionButton style={!quote.canPurchase ? { opacity: .52 } : {}} accent={item.currency === 'gold' ? '#e8c98a' : '#5fd8ff'} onClick={() => onApply(current => purchaseItem(current, item.id, quantity), purchaseSuccessMessage(`${text.name} ×${quantity}`, lang))}>{item.currency === 'gold' ? '🪙' : '💎'} {quote.totalCost || item.cost}</ActionButton>{!quote.canPurchase && <small>{quote.error === 'inventory_full' ? (lang === 'zh' ? '库存已满' : 'FULL') : (lang === 'zh' ? '资源不足' : 'INSUFFICIENT FUNDS')}</small>}</div> : item.id === 'energy_cell' ? <ActionButton disabled={busy || Number(profile.inventory.energy_cell) < 1 || profile.energy >= ENERGY_OVERFLOW_MAX} onClick={() => onApply(current => consumeEnergyCell(current), lang === 'zh' ? '体力 +30' : 'Energy +30')}>{tx.use}</ActionButton> : <ActionButton onClick={() => onApply(current => toggleEquipItem(current, item.id), equipped ? (lang === 'zh' ? '已卸下' : 'Removed') : (lang === 'zh' ? '已装备' : 'Equipped'))}>{equipped ? tx.unequip : tx.equip}</ActionButton>}</div></Panel>;
    })}</div>
