...
export default function ResourceBar({ profile, onCheckin, canCheckin }) {
  const items = [
    { icon: '⚡', color: '#ffd34d', val: `${profile.energy}/${ENERGY_MAX}` },
    { icon: '💎', color: '#5fd8ff', val: fmt(profile.diamonds) },
    { icon: '🪙', color: '#e8c98a', val: fmt(profile.gold) },
  ];
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontFamily: 'monospace' }}>
      {items.map(it => (
        <div key={it.icon} style={{
          ...
          <span onClick={canCheckin ? onCheckin : undefined}
            title={canCheckin ? '每日签到领取' : '今日已签到'}
            style={{
              ...
              background: canCheckin ? `${it.color}22` : 'transparent',
              cursor: canCheckin ? 'pointer' : 'default', opacity: canCheckin ? 1 : 0.3,
              boxShadow: canCheckin ? `0 0 10px ${it.color}55` : 'none',
            }}>+</span>
        </div>
      ))}
    </div>
