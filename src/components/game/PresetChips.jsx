  ...
  return (
    <div style={{
      ...
      {PRESET_CONFIGS.map(p => (
        <button key={p.id} onClick={() => handle(p)} title={p.desc} style={{
          ...
          <span style={{ fontSize: '0.48rem', fontWeight: 900, whiteSpace: 'nowrap' }}>{p.icon} {p.name}</span>
          <span style={{ fontSize: '0.34rem', color: 'rgba(255,255,255,0.32)', whiteSpace: 'nowrap' }}>{p.desc}</span>
        </button>
      ))}
    </div>
  );
