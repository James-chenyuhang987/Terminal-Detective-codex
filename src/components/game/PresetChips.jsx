    ...
    <div style={{
      ...
      {PRESET_CONFIGS.map(p => (
        <button key={p.id} onClick={() => handle(p)} title={zh ? p.desc : p.descEn} style={{
          ...
          border: `1px solid ${noirColor(p.color)}${applied === p.id ? 'ff' : '45'}`,
          background: applied === p.id ? `${noirColor(p.color)}30` : `${noirColor(p.color)}0c`,
          color: noirColor(p.color), fontFamily: 'monospace',
          boxShadow: applied === p.id ? `0 0 14px ${noirColor(p.color)}80` : 'none',
          transform: applied === p.id ? 'scale(0.95)' : 'scale(1)',
          transition: 'all 0.18s',
        }}>
          <span style={{ fontSize: '0.48rem', fontWeight: 900, whiteSpace: 'nowrap' }}><Icon name={p.icon} /> {zh ? p.name : p.nameEn}</span>
