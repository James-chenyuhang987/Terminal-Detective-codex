...
import { firebaseAuthEmailSender } from '@/lib/firebase';
...
const COPY = {
  zh: {
    ...
    resetDesc: '输入邮箱后，我们会发送安全的密码重置邮件。', resetBtn: '发送重置邮件',
    resetSent: '如果该邮箱已注册，密码重置邮件将会送达；请同时检查垃圾邮件和广告邮件。',
    verifyTitle: '等待邮箱验证', verifyDesc: '验证邮件已经发送至',
    verifyHint: '请打开最新一封邮件完成验证，然后返回这里继续。邮件可能需要几分钟送达。', verifiedBtn: '我已完成验证',
    resend: '重新发送验证邮件', changeEmail: '换一个邮箱', resent: '验证邮件已重新发送。',
    mailHelpTitle: '没有看到邮件？',
    mailHelpSteps: ['检查垃圾邮件、广告邮件或“推广”分类。', '搜索下方发件地址；找到后标记为“不是垃圾邮件”。', '将发件地址加入联系人或允许列表，再等待倒计时结束后重发。'],
    mailSender: '常用发件地址',
    ...
    provider_disabled: '该登录方式尚未在 Firebase 启用。',
  ...
  en: {
    ...
    resetDesc: 'Enter your email to receive a secure password reset message.', resetBtn: 'SEND RESET EMAIL',
    resetSent: 'If the email is registered, a password reset message will arrive. Check Spam, Junk, and Promotions too.',
    verifyTitle: 'Verify Your Email', verifyDesc: 'A verification message was sent to',
    verifyHint: 'Open the latest email, finish verification, then return here. Delivery can take a few minutes.', verifiedBtn: 'I HAVE VERIFIED',
    resend: 'RESEND VERIFICATION', changeEmail: 'USE ANOTHER EMAIL', resent: 'Verification email sent again.',
    mailHelpTitle: 'Cannot find the message?',
    mailHelpSteps: ['Check Spam, Junk, or the Promotions tab.', 'Search for the sender below, then mark the message as not spam.', 'Add the sender to contacts or your allowlist before resending.'],
    mailSender: 'Usual sender',
    ...
    provider_disabled: 'This sign-in provider is not enabled in Firebase.',
...
function Feedback({ value, text }) {
  ...
  return <p className={`auth-feedback ${success ? 'success' : 'error'}`} role={success ? 'status' : 'alert'}><Icon name={success ? 'check' : 'warning'} /> {value.message || text[value.code] || text.unknown}</p>;
}

function MailDeliveryHelp({ text }) {
  if (!firebaseAuthEmailSender) return null;
  return (
    <aside className="auth-mail-help" role="note">
      <strong>{text.mailHelpTitle}</strong>
      <ol>{text.mailHelpSteps.map(step => <li key={step}>{step}</li>)}</ol>
      <span>{text.mailSender}: <code>{firebaseAuthEmailSender}</code></span>
    </aside>
  );
}
  ...
  return (
    <main className="auth-gate td-page-shell">
      ...
      <section className="auth-panel td-ui-card" aria-busy={busy}>
        <div className="auth-orb" aria-hidden="true"><span><Icon name="fingerprint" size={28} /></span></div>
        <p className="auth-eyebrow">{text.eyebrow}</p><SlicedTitle as="h1">{mode === 'reset' ? text.resetTitle : mode === 'verify' ? text.verifyTitle : text.title}</SlicedTitle>
        ...
        {mode === 'verify' ? (
          ...
          <form onSubmit={submitReset}>
            ...
            {feedback?.kind === 'success' && <MailDeliveryHelp text={text} />}
        ...
        <p className="auth-security"><Icon name="lock" /> {text.security}</p>
      </section>
      <style>{`
        .auth-gate{min-height:100dvh;display:grid;place-items:center;position:relative;overflow:hidden;padding:max(24px,env(safe-area-inset-top)) max(24px,env(safe-area-inset-right)) max(24px,env(safe-area-inset-bottom)) max(24px,env(safe-area-inset-left));background:radial-gradient(circle at 50% 15%,#17335b 0,#08121c 43%,#08121c 100%);color:#e6dfcf;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.auth-grid{position:absolute;inset:0;opacity:.16;background-image:linear-gradient(rgba(112, 159, 154,.18) 1px,transparent 1px),linear-gradient(90deg,rgba(112, 159, 154,.18) 1px,transparent 1px);background-size:42px 42px;mask-image:linear-gradient(to bottom,#000,transparent 92%)}.auth-scan{position:absolute;inset:-40% 0 auto;height:42%;background:linear-gradient(transparent,rgba(112, 159, 154,.05),transparent);animation:authScan 7s linear infinite;pointer-events:none}.auth-lang{position:absolute;top:max(20px,env(safe-area-inset-top));right:max(22px,env(safe-area-inset-right));z-index:2;min-width:50px;min-height:44px;border:1px solid #709f9a59;border-radius:999px;background:#000a16b3;color:#a5c8c0;font:inherit;cursor:pointer}.auth-panel{width:min(100%,500px);max-height:calc(100dvh - max(32px,env(safe-area-inset-top)) - max(32px,env(safe-area-inset-bottom)));overflow:auto;position:relative;z-index:1;padding:32px 38px;border:1px solid #709f9a4d;border-radius:24px;background:linear-gradient(145deg,#071226f2,#020712f7);box-shadow:0 28px 90px #0009,0 0 50px #709f9a17,inset 0 1px #fff0d;backdrop-filter:blur(18px)}.auth-orb{width:54px;height:54px;display:grid;place-items:center;margin:0 auto 16px;border:1px solid #709f9a80;border-radius:17px;color:#709f9a;font-size:25px;box-shadow:0 0 30px #709f9a38;transform:rotate(45deg)}.auth-orb span{transform:rotate(-45deg)}.auth-eyebrow{margin:0 0 7px;text-align:center;color:#709f9ab0;font-size:9px;letter-spacing:.22em}.auth-panel h1{margin:0;text-align:center;font-size:clamp(24px,5vw,34px);letter-spacing:.06em;background:linear-gradient(90deg,#a5c8c0,#9b9aae);background-clip:text;color:transparent}.auth-subtitle{margin:12px auto 20px;max-width:400px;text-align:center;color:#9eb4c8;font-size:11px;line-height:1.7}.auth-tabs{display:grid;grid-template-columns:1fr 1fr;margin:17px 0;padding:4px;border:1px solid #ffffff12;border-radius:11px;background:#08121c}.auth-tabs button{min-height:39px;border:0;border-radius:8px;background:transparent;color:#9caaa9;font:700 11px/1 inherit;cursor:pointer}.auth-tabs button[aria-selected=true]{background:#709f9a18;color:#a5c8c0;box-shadow:inset 0 0 0 1px #709f9a52}.auth-panel form{display:grid;gap:12px}.auth-panel label{display:grid;gap:6px;color:#9caaa9;font-size:9px;letter-spacing:.08em}.auth-panel input{width:100%;min-height:45px;padding:0 13px;border:1px solid #4f78964d;border-radius:9px;outline:0;background:#08121c;color:#e6dfcf;font:12px/1 inherit}.auth-panel input:focus{border-color:#709f9aaa;box-shadow:0 0 0 3px #709f9a12}.auth-password{display:flex;position:relative}.auth-password input{padding-right:67px}.auth-password button{position:absolute;right:5px;top:5px;bottom:5px;border:0;border-left:1px solid #ffffff16;background:transparent;color:#709f9a;font:8px/1 inherit;cursor:pointer}.auth-primary,.auth-secondary{width:100%;min-height:47px;border-radius:10px;font:800 10px/1.3 inherit;letter-spacing:.1em;cursor:pointer}.auth-primary{border:1px solid #709f9a9e;background:linear-gradient(100deg,#709f9a2e,#9b9aae38);color:#e6dfcf;box-shadow:0 0 25px #709f9a14}.auth-secondary{border:1px solid #4f78965c;background:#101e2a;color:#a5c8c0}.auth-primary:disabled,.auth-secondary:disabled,.auth-github:disabled{cursor:not-allowed;opacity:.42}.auth-forgot,.auth-link{justify-self:end;border:0;background:transparent;color:#709f9a;font:9px/1.4 inherit;cursor:pointer}.auth-link{justify-self:center;margin-top:2px}.auth-rules{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin:-3px 0 1px;color:#d69b63;font-size:8px;line-height:1.4}.auth-rules span{padding:5px 6px;border:1px solid #d69b6330;border-radius:6px}.auth-rules span.valid{border-color:#8aaa9145;color:#8aaa91}.auth-divider{display:flex;align-items:center;gap:10px;margin:18px 0;color:#9caaa9;font-size:8px}.auth-divider:before,.auth-divider:after{content:"";height:1px;flex:1;background:#ffffff12}.auth-github{width:100%;min-height:60px;display:flex;align-items:center;gap:12px;padding:9px 13px;text-align:left;border:1px solid #9b9aae75;border-radius:11px;color:#e6dfcf;background:linear-gradient(100deg,#101e2a,#26194380);cursor:pointer}.auth-github>b{width:38px;height:38px;display:grid;place-items:center;border-radius:50%;background:#e6dfcf;color:#101e2a;font-size:11px}.auth-github>span{flex:1;display:grid;gap:4px}.auth-github strong{font-size:10px}.auth-github small{color:#9caaa9;font-size:8px}.auth-github i{font-style:normal;font-size:21px;color:#9b9aae}.auth-verify{display:grid;gap:12px;margin-top:18px;text-align:center}.auth-verify p{margin:0;color:#91aabe;font-size:10px;line-height:1.65}.auth-verify strong{color:#a5c8c0;font-size:12px;overflow-wrap:anywhere}.auth-mail-help{display:grid;gap:7px;padding:11px 12px;border:1px solid #d6a24d4d;border-radius:9px;background:#2b1c074d;text-align:left;color:#c4ac82;font-size:8px;line-height:1.55}.auth-mail-help>strong{color:#c5a66f;font-size:9px}.auth-mail-help ol{display:grid;gap:3px;margin:0;padding-left:18px}.auth-mail-help code{color:#a5c8c0;overflow-wrap:anywhere}.auth-feedback{margin:13px 0 0;padding:10px 12px;border-radius:9px;text-align:center;font-size:9px;line-height:1.55}.auth-feedback.error{border:1px solid #c77c7859;background:#46041252;color:#dda29a}.auth-feedback.success{border:1px solid #8aaa9159;background:#042e205c;color:#8aaa91}.auth-working{margin:12px 0 0;text-align:center;color:#a5c8c0;font-size:9px}.auth-security{margin:20px 0 0;border-top:1px solid #ffffff12;padding-top:14px;text-align:center;color:#9caaa9;font-size:8px;line-height:1.6}@keyframes authScan{to{transform:translateY(340%)}}@media(max-width:520px){.auth-gate{padding:max(10px,env(safe-area-inset-top)) max(10px,env(safe-area-inset-right)) max(10px,env(safe-area-inset-bottom)) max(10px,env(safe-area-inset-left))}.auth-panel{padding:25px 18px;border-radius:18px;max-height:calc(100dvh - max(20px,env(safe-area-inset-top)) - max(20px,env(safe-area-inset-bottom)))}.auth-lang{top:max(12px,env(safe-area-inset-top));right:max(12px,env(safe-area-inset-right))}.auth-rules{grid-template-columns:1fr}}@media(prefers-reduced-motion:reduce){.auth-scan{animation:none}}
      `}</style>
    </main>
  );
}
