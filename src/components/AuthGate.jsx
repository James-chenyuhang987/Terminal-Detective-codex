        ...
        {mode === 'verify' ? (
          ...
          <>
            ...
            <form onSubmit={submitEmail}>
              ...
              <button className="auth-primary" disabled={busy || !auth.firebaseConfigured} type="submit">{mode === 'register' ? text.registerBtn : text.loginBtn}</button>
            </form>
            <div className="auth-divider"><span>{text.or}</span></div>
            <button className="auth-github" disabled={busy || !auth.firebaseConfigured} onClick={() => void run(() => auth.loginWithGitHub())} type="button"><b>GH</b><span><strong>{text.github}</strong><small>{text.githubHint}</small></span><i>›</i></button>
          </>
        )}
