...
const caseSelect = readSource('../src/components/game/CaseSelect.jsx');
...
test('other ambiguous icon actions include visible names or explicit accessible labels', () => {
  ...
  assert.match(home, /td-home-topbar[\s\S]*zIndex: 30/);
  assert.match(home, /onSelectCase[\s\S]*选择其他案件[\s\S]*选择案件/);
  assert.match(caseSelect, /activeRun[\s\S]*onAbandonRun/);
  assert.match(caseSelect, /结束并选择新案件/);
  assert.match(terminal, /createPlayerRunClient[\s\S]*type: 'abandon'/);
